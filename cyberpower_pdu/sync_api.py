"""A synchronous library for interacting with CyberPower PDU (power distribution units), allowing
...
class CyberPowerPDU(ABC):
    ...
    @abstractmethod
    def get_bank_load(self, bank: int) -> float:
        """Get the total electrical load, in amps, of the given bank"""
        ...

    @abstractmethod
    def get_outlet_state(self, outlet: int) -> bool:
    ...
    @abstractmethod
    def send_outlet_command(self, outlet: int, command: OutletCommand) -> None:
...
class CyberPowerPDUHardware(CyberPowerPDU):
    ...
    def initialize(self) -> None:
        ...
        self.__number_of_banks = get_number_of_banks(self.__ip_address)
        self.__number_of_outlets = get_number_of_outlets(self.__ip_address)
    ...
    def __valid_outlet_index(self, outlet: int) -> bool:
        ...
        return 0 < outlet <= self.__number_of_outlets
    ...
    def __get_outlet_value_error(self, outlet: int) -> ValueError:
        ...
        message = (
            ...
            "are 1 to {self.__number_of_outlets}"
        )
    ...
    def get_bank_load(self, bank: int) -> float:
        ...
        if 0 < bank <= self.__number_of_banks:
            return get_bank_load(self.__ip_address, bank)
        else:
            message = (
    ...
    def get_outlet_state(self, outlet: int) -> bool:
        ...
        if self.__valid_outlet_index(outlet):
            return get_outlet_state(self.__ip_address, outlet)
        else:
    ...
    def send_outlet_command(self, outlet: int, command: OutletCommand) -> None:
        """Send a command to the outlet"""
        if self.__valid_outlet_index(outlet):
            return send_outlet_command(self.__ip_address, outlet, command)
        else:
            raise self.__get_outlet_value_error(outlet)
...
def get_data(ip_address: str, object_identity: str) -> int | str:
    ...
    error_indication, error_status, _error_index, variable_bindings = next(iterator)
...
def get_integer_data(ip_address: str, object_identity: str) -> int:
    """An integer-typed version of `get_data`"""
    response = get_data(ip_address, object_identity)
    match response:
        case int():
            return response
        case _:
...
def get_string_data(ip_address: str, object_identity: str) -> str:
...
def set_data(ip_address: str, object_identity: str, value: Any) -> None:
    ...
    error_indication, error_status, _error_index, _variable_bindings = next(iterator)
...
def get_number_of_outlets(ip_address: str) -> int:
    ...
    return get_integer_data(ip_address, ".1.3.6.1.4.1.3808.1.1.3.3.1.3.0")


def get_number_of_banks(ip_address: str) -> int:
    ...
    return get_integer_data(ip_address, ".1.3.6.1.4.1.3808.1.1.3.2.1.4.0")


def get_outlet_state(ip_address: str, outlet: int) -> bool:
    ...
    response = get_integer_data(ip_address, oid)
    match response:
        case 1:
            return True
        case 2:
...
def send_outlet_command(ip_address: str, outlet: int, command: OutletCommand) -> None:
    ...
    set_data(ip_address, oid, Integer(command.value))


def get_bank_load(ip_address: str, bank: int) -> float:
    """Get the load, in amps, of the bank"""
    # The OID corresponds to ePDU2BankStatusLoad in the CyberPower_MIB_v2.11.mib file
    tenths_of_amps = get_integer_data(ip_address, f".1.3.6.1.4.1.3808.1.1.6.5.4.1.5.{bank}")
